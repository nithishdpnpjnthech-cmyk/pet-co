...
const OutletTravelSafety = ({ initialActive = 'All Travel & Safety' }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let filteredProducts = (apiData || []).filter(item => {
          const categoryMatch = item?.category?.toUpperCase() === 'OUTLET TRAVEL & SAFETY' || 
                               item?.category?.toUpperCase() === 'TRAVEL & SAFETY' ||
                               item?.category?.toUpperCase() === 'TRAVEL';
          
          if (!categoryMatch) return false;
          
          if (active === 'All Travel & Safety') {
            return true;
          }
        ...
        console.log('Filtered travel & safety for', active, ':', normalized);
        setProducts(normalized);
      } catch (err) {
        console.error('Outlet Travel & Safety: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet><title>Outlet Travel & Safety - Pet & Co</title></Helmet>
      <Header />
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Outlet Travel & Safety</h1>
          <p className="text-sm text-muted-foreground mt-2">Discounted travel and safety gear for your pets</p>
        </div>
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                <div className="text-center">
                  ...
                  <div className="text-foreground font-semibold mb-2">No travel & safety products found in "{active}"</div>
...
export default OutletTravelSafety;