...
const OutletAccessories = ({ initialActive = 'All Accessories' }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let filteredProducts = (apiData || []).filter(item => {
          const categoryMatch = item?.category?.toUpperCase() === 'OUTLET ACCESSORIES' || 
                               item?.category?.toUpperCase() === 'ACCESSORIES' ||
                               item?.category?.toUpperCase() === 'ACCESSORY';
          
          if (!categoryMatch) return false;
          
          if (active === 'All Accessories') {
            return true;
          }
        ...
        console.log('Filtered accessories for', active, ':', normalized);
        setProducts(normalized);
      } catch (err) {
        console.error('Outlet Accessories: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                <div className="text-center">
                  ...
                  <div className="text-foreground font-semibold mb-2">No accessories found in "{active}"</div>
...
export default OutletAccessories;