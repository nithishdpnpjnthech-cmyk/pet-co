...
const OutletGrooming = ({ initialActive = 'All Grooming & Care' }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let filteredProducts = (apiData || []).filter(item => {
          const categoryMatch = item?.category?.toUpperCase() === 'OUTLET GROOMING & CARE' || 
                               item?.category?.toUpperCase() === 'GROOMING & CARE' ||
                               item?.category?.toUpperCase() === 'GROOMING';
          
          if (!categoryMatch) return false;
          
          if (active === 'All Grooming & Care') {
            return true;
          }
        ...
        console.log('Filtered grooming products for', active, ':', normalized);
        setProducts(normalized);
      } catch (err) {
        console.error('Outlet Grooming: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet><title>Outlet Grooming & Care - Pet & Co</title></Helmet>
      <Header />
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Outlet Grooming & Care</h1>
          <p className="text-sm text-muted-foreground mt-2">Discounted grooming essentials for your pets</p>
        </div>
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                <div className="text-center">
                  ...
                  <div className="text-foreground font-semibold mb-2">No grooming products found in "{active}"</div>
