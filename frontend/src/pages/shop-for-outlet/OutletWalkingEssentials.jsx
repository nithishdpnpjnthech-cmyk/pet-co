...
const ProductCard = ({ p }) => {
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      quantity: 1,
      category: 'Outlet Walking Essentials'
    });
  };
...
const OutletWalkingEssentials = ({ initialActive = 'All Walking Essentials' }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let filteredProducts = (apiData || []).filter(item => {
          const categoryMatch = item?.category?.toUpperCase() === 'OUTLET WALKING ESSENTIALS' || 
                               item?.category?.toUpperCase() === 'WALKING ESSENTIALS' ||
                               item?.category?.toUpperCase() === 'WALKING';
          
          if (!categoryMatch) return false;
          
          if (active === 'All Walking Essentials') {
            return true;
          }
        ...
        console.log('Filtered walking essentials for', active, ':', normalized);
        setProducts(normalized);
      } catch (err) {
        console.error('Outlet Walking Essentials: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet><title>Outlet Walking Essentials - Pet & Co</title></Helmet>
      <Header />
      <div className="container mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Outlet Walking Essentials</h1>
          <p className="text-sm text-muted-foreground mt-2">Discounted collars, leashes, and harnesses for your pets</p>
        </div>
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                <div className="text-center">
                  ...
                  <div className="text-foreground font-semibold mb-2">No walking essentials found in "{active}"</div>
...
export default OutletWalkingEssentials;