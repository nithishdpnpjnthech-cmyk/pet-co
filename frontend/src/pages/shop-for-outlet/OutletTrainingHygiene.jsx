...
const ProductCard = ({ p }) => {
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      quantity: 1,
      category: 'Outlet Training & Hygiene'
    });
  };
...
const OutletTrainingHygiene = ({ initialActive = 'All Training & Hygiene' }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        let filteredProducts = (apiData || []).filter(item => {
          const categoryMatch = item?.category?.toUpperCase() === 'OUTLET TRAINING & HYGIENE' || 
                               item?.category?.toUpperCase() === 'TRAINING & HYGIENE' ||
                               item?.category?.toUpperCase() === 'TRAINING';
          
          if (!categoryMatch) return false;
          
          if (active === 'All Training & Hygiene') {
            return true;
          }
        ...
        console.log('Filtered training & hygiene for', active, ':', normalized);
        setProducts(normalized);
      } catch (err) {
        console.error('Outlet Training & Hygiene: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet><title>Outlet Training & Hygiene - Pet & Co</title></Helmet>
      <Header />
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                <div className="text-center">
                  ...
                  <div className="text-foreground font-semibold mb-2">No training & hygiene products found in "{active}"</div>
...
export default OutletTrainingHygiene;