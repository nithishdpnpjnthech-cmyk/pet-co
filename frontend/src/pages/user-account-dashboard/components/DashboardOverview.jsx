...
const DashboardOverview = ({ user, recentOrders, loyaltyPoints, wishlistCount, pendingReviewsCount }) => {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: 'Star',
      color: pendingReviewsCount > 0 ? 'text-yellow-500' : 'text-muted-foreground',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats?.map((stat, index) => (
          <div key={index} className={`bg-card border border-border rounded-lg p-4 ${stat.badge ? 'ring-2 ring-yellow-500/20' : ''}`}>
