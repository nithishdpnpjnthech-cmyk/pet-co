...
const CatBowls = ({ initialActive = 'All Cat Bowls' }) => {
  ...
  const routeMap = { 'All Cat Bowls': '/shop-for-cats/cat-bowls/all-cat-bowls', 'Bowls': '/shop-for-cats/cat-bowls/bowls', 'Travel & Fountain': '/shop-for-cats/cat-bowls/travel-fountain', 'Elevated Feeders': '/shop-for-cats/cat-bowls/elevated-feeders', 'Automatic Feeders': '/shop-for-cats/cat-bowls/automatic-feeders' };
  ...
  const productTypes = ['Bowls','Slow Feeders','Fountains','Travel Bowls','Anti Spill Mats'];
  const specialDiets = ['Eco Friendly','BPA Free','Dishwasher Safe'];
  const proteinSource = ['Ceramic','Stainless Steel','Silicone','Plastic'];
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const routeMatch = urlPath.match(/cat-bowls\/(.*)$/i);
        ...
        console.error('Failed to load cat bowls via API', err);
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    const pageCategory = 'Cat Bowls'; // This page's category
  ...
  return (
    <>
      <Helmet><title>Shop for Cats — Cat Bowls</title> <style>{`
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            ...
            {!loading && !error && (filteredProducts.length > 0 ? (
              ...
              <div className="col-span-full py-12 text-center text-muted-foreground">
                No cat bowls match the selected filters.
              </div>
            ))}
          </div>
        </main>
