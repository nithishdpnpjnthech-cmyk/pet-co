...
const sampleProducts = [
  { id: 'l1', name: 'Clumping Cat Litter', image: '/assets/images/essential/meowsi.webp', badges: ['Absorbent'], variants: ['5 L','10 L'], price: 499 },
  { id: 'l2', name: 'Durable Litter Tray', image: '/assets/images/essential/whiskas.webp', badges: ['Easy Clean'], variants: ['Small','Large'], price: 899 },
  { id: 'l3', name: 'Scooper with Filter', image: '/assets/images/essential/sheba.webp', badges: ['Handy'], variants: ['One Size'], price: 149 }
];
...
const CatLitter = ({ initialActive = 'All Litter & Supplies' }) => {
  const [active, setActive] = useState(initialActive);
  const { getCartItemCount, cartItems } = useCart();
  const navigate = useNavigate();
  const leftRef = useRef(null);
  const rightRef = useRef(null);
  const routeMap = { 'All Litter & Supplies': '/shop-for-cats/cat-litter/all-litter-supplies', 'Litter': '/shop-for-cats/cat-litter/litter', 'Litter Trays': '/shop-for-cats/cat-litter/litter-trays', 'Scooper': '/shop-for-cats/cat-litter/scooper', 'Liners & Deodorizers': '/shop-for-cats/cat-litter/liners-deodorizers' };
  ...
  // cat litter specific filter data
  const brands = ['World\'s Best','Dr. Elsey\'s','Tidy Cats','Arm & Hammer','Fresh Step','Purina'];
  ...
  const productTypes = ['Clay Litter','Crystal Litter','Natural Litter','Litter Trays','Scoopers','Mats'];
  const specialDiets = ['Dust-Free','Odor Control','Clumping','Non-Clumping','Flushable'];
  const proteinSource = ['Natural','Clay-Based','Crystal','Wood','Paper'];
  const priceRanges = ['INR 200 - INR 500','INR 501 - INR 1000','INR 1001 - INR 2000','INR 2000+'];
  const weights = ['5 kg','10 kg','15 kg','20 kg'];
  const sizes = ['Small','Medium','Large','Extra Large'];
  const subCategories = ['Clay Litter','Crystal Litter','Natural Litter','Litter Boxes','Scoopers','Accessories'];
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await productApi.getCustomerProducts({ type: 'Cat' });
        ...
        setFetchError('');
      } catch (err) { console.error('Failed to load cat litter', err); setProducts([]); setFetchError('Unable to load cat litter.'); }
  ...
  useEffect(() => {
    ...
    const pageCategory = 'Cat Litter & Supplies'; // This page's category
  ...
  return (
    <>
      <Helmet>
        <title>Shop for Cats — Litter & Supplies</title>
         <style>{`
      ...
      <div className="grid grid-cols-12 gap-3 md:gap-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {loading ? (
              <div className="col-span-full py-12 text-center text-muted-foreground">Loading cat litter...</div>
