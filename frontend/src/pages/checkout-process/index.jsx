...
const CheckoutProcess = () => {
  ...
  const { cartItems, clearCart, addToCart, appliedCoupon, couponDiscount, applyCoupon, removeCoupon } = useCart();
  ...
  // coupon state handled by CartContext
  ...
  useEffect(() => {
    if (!loading && user && (!cartItems || cartItems.length === 0) && !showOrderSuccessModal) {
      setShowEmptyCartMessage(true);
    } else {
      setShowEmptyCartMessage(false);
    }
  }, [cartItems, user, loading, showOrderSuccessModal]);
  ...
  // Test function to show success modal (for development/testing)
  const testSuccessModal = () => {
    setSuccessOrderData({
      ...
      items: cartItems || [{ name: 'Test Product' }],
      user: user
    });
  ...
  // Remove legacy auto-apply; coupons administered via CartContext validation
   ...
   * For online payments, the order is already created by the payment verification flow
   */
  const handlePlaceOrder = async (paymentResult = null) => {
    ...
    try {
      ...
      // If paymentResult is provided, it means online payment was successful
      // and the order was already created during payment verification
      if (paymentResult && paymentResult.order) {
        console.log('Using order from payment verification:', paymentResult.order);
        savedOrder = paymentResult.order;
      } else {
        // For COD payments, place order through regular backend API
        savedOrder = await checkoutApi.placeOrder(user.email);
        console.log('Order placed successfully:', savedOrder);
      }

      // Skip WhatsApp notification; show success message only

      // Resolve id, total and items from backend response
      const resolvedId = savedOrder?.id ?? savedOrder?.orderId ?? savedOrder?.razorpayOrderId ?? null;
      const resolvedTotal = Number(
        (savedOrder && (savedOrder.total ?? savedOrder.totalAmount ?? savedOrder.amount))
        ?? (orderReviewData && orderReviewData.total)
        ?? 0
      );
      const resolvedItems = savedOrder?.items || savedOrder?.orderItems || orderReviewData?.items || cartItems;
      ...
      setSuccessOrderData({
        ...
        customerName: user?.name || shippingData?.fullName,
        items: resolvedItems,
        user: user
      });
      setShowOrderSuccessModal(true);

      // Clear cart after showing success modal
      clearCart();
  ...
  // WhatsApp notifications removed per requirement
  ...
  const handleCloseSuccessModal = () => {
    setShowOrderSuccessModal(false);
    // Auto-redirect to orders after a delay if user doesn't choose
    setTimeout(() => {
      navigate('/user-account-dashboard?section=orders');
    }, 2000);
  };
  ...
  const handleApplyCoupon = async (couponCode) => {
    ...
    await applyCoupon({ code: couponCode, context: { /* optional: set petType/category/subcategory if needed */ } });
  };
  ...
  if (showEmptyCartMessage && !showOrderSuccessModal) {
    return (
